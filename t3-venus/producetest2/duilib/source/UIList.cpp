...
void CListUI::SetPos(RECT rc)
{
    ...
    if( !m_pHeader->IsVisible() ) {
        ...
        m_pHeader->SetPos(CDirectRect(rc.left, 0, rc.right, 0));
    }
...
bool CListUI::SelectItem(int iIndex)
{
    ...
    //pControl->SetFocus();
    if( m_pManager != NULL ) {
        m_pManager->SendNotify(this, NOTIFY_ITEMSELECT, m_iCurSel, iOldSel);
    }
...
void CListUI::Scroll(int dx, int dy)
{
    ...
    m_pList->SetScrollPos(CDirectSize(sz.cx + dx, sz.cy + dy));
}
...
void CListBodyUI::SetPos(RECT rc)
{
    ...
    if( m_pOwner ) {
        ...
        if( pHeader != NULL && pHeader->GetCount() > 0 ) {
            cxNeeded = MAX(0, pHeader->EstimateSize(CDirectSize(rc.right - rc.left, rc.bottom - rc.top)).cx);
        }
    }
...
{
	SetTextPadding(CDirectRect(2, 0, 2, 0));
...
void CListHeaderItemUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK )
    {
        ...
        if( ::PtInRect(&rcSeparator, event.ptMouse) ) {
            ...
            m_pManager->SendNotify(this, NOTIFY_HEADERCLICK );
            Invalidate();
        }
        return;
    }
...
{
    if( m_cxyFixed.cy == 0 ) return CDirectSize(m_cxyFixed.cx, m_pManager->GetDefaultFontInfo()->tm.tmHeight + 14);
...
bool CListElementUI::Activate()
{
    ...
    if( m_pManager != NULL ) m_pManager->SendNotify(this, NOTIFY_ITEMACTIVATE);
    return true;
}
...
void CListLabelElementUI::DoEvent(TEventUI& event)
{
    ...
    {
        if( IsEnabled() ) {
            m_pManager->SendNotify(this, NOTIFY_ITEMCLICK);
            Select();
            Invalidate();
        }
        return;
...
{
    if( m_pOwner == NULL ) return CDirectSize(0, 0);
...
void CListTextElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
{
    ...
    for( i = m_nLinks; i < lengthof(m_rcLinks); i++ ) {
