...
void CComboWnd::Init(CComboUI* pOwner)
{
    ...
    CDirectRect rcWork = oMonitor.rcWork;
...
LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if( uMsg == WM_CREATE ) {
        ...
        m_pLayout->SetInset(CDirectRect(2, 2, 2, 2));
		...
		//let the dropbox proc wheel event itself --- by zh. 20130227
		int zDelta = (int) (short) HIWORD(wParam);
        DWORD dwMode = MAKELPARAM(zDelta < 0 ? SB_LINEDOWN : SB_LINEUP, 0);
		bool bDownward = dwMode == SB_LINEDOWN;
		Scroll(0,(bDownward ? 10: -10 ));
        return 0;
    }
...
void CComboWnd::Scroll(int dx, int dy)
{
    ...
    m_pLayout->SetScrollPos(CDirectSize(sz.cx + dx, sz.cy + dy));
}
...
{
	//VK_UP/VK_DOWN on dropbox, select item without close it --- by zh. 20130227
    ...
    if( m_pManager != NULL ) m_pManager->SendNotify(this, NOTIFY_ITEMSELECT, m_iCurSel, iOldSel);
	if ( m_pWindow != NULL )
	{
		m_pWindow->EnsureVisible( m_iCurSel );
	}
    Invalidate();
...
{
    if( m_cxyFixed.cy == 0 ) return CDirectSize(m_cxyFixed.cx, m_pManager->GetDefaultFontInfo()->tm.tmHeight + 12);
...
bool CComboUI::Activate()
{
    ...
    ASSERT(m_pWindow);
    m_pWindow->Init(this);
    if( m_pManager != NULL ) m_pManager->SendNotify(this, NOTIFY_DROPDOWN );
    Invalidate();
    return true;
}
