        ...
        if( _tcscmp(pstrClass, _T("Include")) == 0 ) {
            ...
            for ( int i = 0; i < count; i++ ) {
                ...
                if( m_pstrtype != NULL ) { // 使用资源dll，从资源中读取
                    ...
                    pControl = builder.Create((LPCTSTR)szValue, 0, m_pCallback, pManager, pParent);
                }
            }
            continue;
        }
...
CControlUI* CDialogBuilder::Create( CMarkupNode* pNode, CControlUI* pParent /* = NULL */, CPaintManagerUI* pManager /* = NULL */ )
{
	return _Parse( pNode, pParent, pManager );
}
