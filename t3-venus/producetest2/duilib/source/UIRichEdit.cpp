...
//
//
const CStdString CRichEditUI::st_str_vscrollbar			=_T("vscrollbar");
const CStdString CRichEditUI::st_str_autovscroll		=_T("autovscroll");
const CStdString CRichEditUI::st_str_hscrollbar			=_T("hscrollbar");
const CStdString CRichEditUI::st_str_autohscroll		=_T("autohscroll"); 
const CStdString CRichEditUI::st_str_wanttab			=_T("wanttab");
const CStdString CRichEditUI::st_str_wantreturn			=_T("wantreturn");
const CStdString CRichEditUI::st_str_wantctrlreturn		=_T("wantctrlreturn");
const CStdString CRichEditUI::st_str_rich				=_T("rich");
const CStdString CRichEditUI::st_str_multiline			=_T("multiline");
const CStdString CRichEditUI::st_str_readonly			=_T("readonly");
const CStdString CRichEditUI::st_str_numberonly			=_T("numberonly");
const CStdString CRichEditUI::st_str_password			=_T("password");
const CStdString CRichEditUI::st_str_align				=_T("align");
const CStdString CRichEditUI::st_str_maxchar			=_T("maxchar");
const CStdString CRichEditUI::st_str_font				=_T("font");
const CStdString CRichEditUI::st_str_textcolor			=_T("textcolor");
...
CDirectPoint CRichEditUI::GetCharPos(long lChar) const
{ 
    CDirectPoint pt; 
...
CDirectPoint CRichEditUI::PosFromChar(UINT nChar) const
{ 
    POINTL pt; 
    TxSendMessage(EM_POSFROMCHAR, (WPARAM)&pt, nChar, 0); 
    return CDirectPoint(pt.x, pt.y); 
}

int CRichEditUI::CharFromPos(CDirectPoint pt) const
{ 
    ...
    if( m_pTwh ) {
        if( msg == WM_KEYDOWN && TCHAR(wparam) == VK_RETURN ) {
            if( !m_bWantReturn || (::GetKeyState(VK_CONTROL) < 0 && !m_bWantCtrlReturn) ) {
                if( m_pManager != NULL ) m_pManager->SendNotify((CControlUI*)this, NOTIFY_RETURN);
                return S_OK;
            }
        }
...
void CRichEditUI::DoEvent(TEventUI& event)
{
	...
	if ( event.Type == UIEVENT_KEYDOWN )
	{
		if ( m_pManager != NULL )
		{
			m_pManager->SendNotify( this, NOTIFY_KEYDOWN, (WPARAM)event.chKey, 0 , false);
		}
	}
	if ( event.Type == UIEVENT_KEYUP )
	{
		if ( m_pManager != NULL )
		{
			m_pManager->SendNotify( this, NOTIFY_KEYUP, (WPARAM)event.chKey, 0 , false);
		}
	}
	if ( event.Type == UIEVENT_CHAR )
	{
		if ( m_pManager != NULL )
		{
			m_pManager->SendNotify( this, NOTIFY_CHAR, (WPARAM)event.chKey, 0 , false);
		}
	}
...
{
    if( _tcscmp(pstrName, CRichEditUI::st_str_vscrollbar ) == 0 ) {
        if( _tcscmp(pstrValue, _T("true")) == 0 ) m_lTwhStyle |= ES_DISABLENOSCROLL | WS_VSCROLL;
    }
    if( _tcscmp(pstrName, CRichEditUI::st_str_autovscroll ) == 0 ) {
        if( _tcscmp(pstrValue, _T("true")) == 0 ) m_lTwhStyle |= ES_AUTOVSCROLL;
    }
    else if( _tcscmp(pstrName, CRichEditUI::st_str_hscrollbar ) == 0 ) {
        if( _tcscmp(pstrValue, _T("true")) == 0 ) m_lTwhStyle |= ES_DISABLENOSCROLL | WS_HSCROLL;
    }
    if( _tcscmp(pstrName, CRichEditUI::st_str_autohscroll ) == 0 ) {
        if( _tcscmp(pstrValue, _T("true")) == 0 ) m_lTwhStyle |= ES_AUTOHSCROLL;
    }
    else if( _tcscmp(pstrName, CRichEditUI::st_str_wanttab ) == 0 ) {
        SetWantTab(_tcscmp(pstrValue, _T("true")) == 0);
    }
    else if( _tcscmp(pstrName, CRichEditUI::st_str_wantreturn ) == 0 ) {
        SetWantReturn(_tcscmp(pstrValue, _T("true")) == 0);
    }
    else if( _tcscmp(pstrName, CRichEditUI::st_str_wantctrlreturn ) == 0 ) {
        SetWantCtrlReturn(_tcscmp(pstrValue, _T("true")) == 0);
    }
    else if( _tcscmp(pstrName, CRichEditUI::st_str_rich ) == 0 ) {
        SetRich(_tcscmp(pstrValue, _T("true")) == 0);
    }
    else if( _tcscmp(pstrName, CRichEditUI::st_str_multiline ) == 0 ) {
        if( _tcscmp(pstrValue, _T("false")) == 0 ) m_lTwhStyle &= ~ES_MULTILINE;
    }
    else if( _tcscmp(pstrName, CRichEditUI::st_str_readonly ) == 0 ) {
        if( _tcscmp(pstrValue, _T("true")) == 0 ) { m_lTwhStyle |= ES_READONLY; m_bReadOnly = true; }
    }
    else if( _tcscmp(pstrName, CRichEditUI::st_str_numberonly ) == 0 ) {
        if( _tcscmp(pstrValue, _T("true")) == 0 ) m_lTwhStyle |= ES_NUMBER;
    }
    else if( _tcscmp(pstrName, CRichEditUI::st_str_password ) == 0 ) {
        if( _tcscmp(pstrValue, _T("true")) == 0 ) m_lTwhStyle |= ES_PASSWORD;
    }
    else if( _tcscmp(pstrName, CRichEditUI::st_str_align ) == 0 ) {
	...
	else if( _tcscmp(pstrName, CRichEditUI::st_str_maxchar ) == 0 ) {
        SetLimitText(_ttoi(pstrValue));
    }

    else if( _tcscmp(pstrName, CRichEditUI::st_str_font ) == 0 ) SetFont(_ttoi(pstrValue));
    else if( _tcscmp(pstrName, CRichEditUI::st_str_textcolor ) == 0 ) {
