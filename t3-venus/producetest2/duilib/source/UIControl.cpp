...
namespace DuiLib {
	...
	const CStdString CControlUI::st_str_pos					=_T("pos");
	const CStdString CControlUI::st_str_relativepos			=_T("relativepos");
	const CStdString CControlUI::st_str_padding				=_T("padding");
	const CStdString CControlUI::st_str_bkcolor				=_T("bkcolor");
	const CStdString CControlUI::st_str_bkcolor1			=_T("bkcolor1");
	const CStdString CControlUI::st_str_bkcolor2			=_T("bkcolor2");
	const CStdString CControlUI::st_str_bkcolor3			=_T("bkcolor3");
	const CStdString CControlUI::st_str_bordercolor			=_T("bordercolor");
	const CStdString CControlUI::st_str_focusbordercolor	=_T("focusbordercolor");
	const CStdString CControlUI::st_str_colorhsl			=_T("colorhsl");
	const CStdString CControlUI::st_str_bordersize			=_T("bordersize");
	const CStdString CControlUI::st_str_borderround			=_T("borderround");
	const CStdString CControlUI::st_str_bkimage				=_T("bkimage");
	const CStdString CControlUI::st_str_width				=_T("width");
	const CStdString CControlUI::st_str_height				=_T("height");
	const CStdString CControlUI::st_str_minwidth			=_T("minwidth");
	const CStdString CControlUI::st_str_minheight			=_T("minheight");
	const CStdString CControlUI::st_str_maxwidth			=_T("maxwidth");
	const CStdString CControlUI::st_str_maxheight			=_T("maxheight");
	const CStdString CControlUI::st_str_name				=_T("name");
	const CStdString CControlUI::st_str_text				=_T("text");
	const CStdString CControlUI::st_str_tooltip				=_T("tooltip");
	const CStdString CControlUI::st_str_userdata			=_T("userdata"); 
	const CStdString CControlUI::st_str_enabled				=_T("enabled");
	const CStdString CControlUI::st_str_mouse				=_T("mouse");
	const CStdString CControlUI::st_str_visible				=_T("visible");
	const CStdString CControlUI::st_str_float				=_T("float");
	const CStdString CControlUI::st_str_shortcut			=_T("shortcut");
	const CStdString CControlUI::st_str_menu				=_T("menu");
...
void CControlUI::SetPos(RECT rc)
{
    ...
    CDirectRect invalidateRc = m_rcItem;
...
void CControlUI::DoEvent(TEventUI& event)
{
    ...
    {
        m_pManager->SendNotify(this, NOTIFY_TIMER, event.wParam, event.lParam);
        return;
    }
...
{
    if( _tcscmp(pstrName, CControlUI::st_str_pos) == 0 ) {
        RECT rcPos = { 0 };
        LPTSTR pstr = NULL;
    ...
    else if( _tcscmp(pstrName, CControlUI::st_str_relativepos) == 0 ) {
        SIZE szMove,szZoom;
        LPTSTR pstr = NULL;
    ...
    else if( _tcscmp(pstrName, CControlUI::st_str_padding) == 0 ) {
        RECT rcPadding = { 0 };
        LPTSTR pstr = NULL;
    ...
    else if( _tcscmp(pstrName, CControlUI::st_str_bkcolor) == 0 || _tcscmp(pstrName, CControlUI::st_str_bkcolor1) == 0 ) {
    ...
    else if( _tcscmp(pstrName, CControlUI::st_str_bkcolor2) == 0 ) {
        while( *pstrValue > _T('\0') && *pstrValue <= _T(' ') ) pstrValue = ::CharNext(pstrValue);
        if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
        LPTSTR pstr = NULL;
        DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
    ...
    else if( _tcscmp(pstrName, CControlUI::st_str_bkcolor3) == 0 ) {
        while( *pstrValue > _T('\0') && *pstrValue <= _T(' ') ) pstrValue = ::CharNext(pstrValue);
        if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
        LPTSTR pstr = NULL;
        DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
    ...
    else if( _tcscmp(pstrName, CControlUI::st_str_bordercolor) == 0 ) {
        if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
        LPTSTR pstr = NULL;
        DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
        SetBorderColor(clrColor);
    }
    else if( _tcscmp(pstrName, CControlUI::st_str_focusbordercolor) == 0 ) {
        if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
        LPTSTR pstr = NULL;
        DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
        SetFocusBorderColor(clrColor);
    }
    else if( _tcscmp(pstrName, CControlUI::st_str_colorhsl ) == 0 ) SetColorHSL(_tcscmp(pstrValue, _T("true")) == 0);
    else if( _tcscmp(pstrName, CControlUI::st_str_bordersize) == 0 ) SetBorderSize(_ttoi(pstrValue));
    else if( _tcscmp(pstrName, CControlUI::st_str_borderround) == 0 ) {
        SIZE cxyRound = { 0 };
        LPTSTR pstr = NULL;
    ...
    else if( _tcscmp(pstrName, CControlUI::st_str_bkimage) == 0 ) SetBkImage(pstrValue);
    else if( _tcscmp(pstrName, CControlUI::st_str_width) == 0 ) SetFixedWidth(_ttoi(pstrValue));
    else if( _tcscmp(pstrName, CControlUI::st_str_height) == 0 ) SetFixedHeight(_ttoi(pstrValue));
    else if( _tcscmp(pstrName, CControlUI::st_str_minwidth) == 0 ) SetMinWidth(_ttoi(pstrValue));
    else if( _tcscmp(pstrName, CControlUI::st_str_minheight) == 0 ) SetMinHeight(_ttoi(pstrValue));
    else if( _tcscmp(pstrName, CControlUI::st_str_maxwidth) == 0 ) SetMaxWidth(_ttoi(pstrValue));
    else if( _tcscmp(pstrName, CControlUI::st_str_maxheight) == 0 ) SetMaxHeight(_ttoi(pstrValue));
    else if( _tcscmp(pstrName, CControlUI::st_str_name) == 0 ) SetName(pstrValue);
    else if( _tcscmp(pstrName, CControlUI::st_str_text) == 0 ) SetText(pstrValue);
    else if( _tcscmp(pstrName, CControlUI::st_str_tooltip) == 0 ) SetToolTip(pstrValue);
    else if( _tcscmp(pstrName, CControlUI::st_str_userdata) == 0 ) SetUserData(pstrValue);
    else if( _tcscmp(pstrName, CControlUI::st_str_enabled) == 0 ) SetEnabled(_tcscmp(pstrValue, _T("true")) == 0);
    else if( _tcscmp(pstrName, CControlUI::st_str_mouse) == 0 ) SetMouseEnabled(_tcscmp(pstrValue, _T("true")) == 0);
    else if( _tcscmp(pstrName, CControlUI::st_str_visible) == 0 ) SetVisible(_tcscmp(pstrValue, _T("true")) == 0);
    else if( _tcscmp(pstrName, CControlUI::st_str_float) == 0 ) SetFloat(_tcscmp(pstrValue, _T("true")) == 0);
    else if( _tcscmp(pstrName, CControlUI::st_str_shortcut) == 0 ) SetShortcut(pstrValue[0]);
    else if( _tcscmp(pstrName, CControlUI::st_str_menu) == 0 ) SetContextMenuUsed(_tcscmp(pstrValue, _T("true")) == 0);
}
