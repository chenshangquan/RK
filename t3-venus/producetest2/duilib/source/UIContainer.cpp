...
namespace DuiLib {
	...
	//
	//
	const CStdString CContainerUI::st_str_inset				=_T("inset");
	const CStdString CContainerUI::st_str_mousechild		=_T("mousechild");
	const CStdString CContainerUI::st_str_vscrollbar		=_T("vscrollbar");
	const CStdString CContainerUI::st_str_vscrollbarstyle	=_T("vscrollbarstyle");
	const CStdString CContainerUI::st_str_hscrollbar		=_T("hscrollbar");
	const CStdString CContainerUI::st_str_hscrollbarstyle	=_T("hscrollbarstyle");
	const CStdString CContainerUI::st_str_childpadding		=_T("childpadding");
...
void CContainerUI::DoEvent(TEventUI& event)
{
	...
	{
		m_pManager->SendNotify( this, NOTIFY_DBLCLICK );
	}
...
{
    if( _tcscmp(pstrName, CContainerUI::st_str_inset ) == 0 ) {
        RECT rcInset = { 0 };
        LPTSTR pstr = NULL;
    ...
    else if( _tcscmp(pstrName, CContainerUI::st_str_mousechild ) == 0 ) SetMouseChildEnabled(_tcscmp(pstrValue, _T("true")) == 0);
    else if( _tcscmp(pstrName, CContainerUI::st_str_vscrollbar ) == 0 ) {
        EnableScrollBar(_tcscmp(pstrValue, _T("true")) == 0, GetHorizontalScrollBar() != NULL);
    }
    else if( _tcscmp(pstrName, CContainerUI::st_str_vscrollbarstyle ) == 0 ) {
        EnableScrollBar(true, GetHorizontalScrollBar() != NULL);
        if( GetVerticalScrollBar() ) GetVerticalScrollBar()->ApplyAttributeList(pstrValue);
    }
    else if( _tcscmp(pstrName, CContainerUI::st_str_hscrollbar ) == 0 ) {
        EnableScrollBar(GetVerticalScrollBar() != NULL, _tcscmp(pstrValue, _T("true")) == 0);
    }
    else if( _tcscmp(pstrName, CContainerUI::st_str_hscrollbarstyle ) == 0 ) {
    ...
    else if( _tcscmp(pstrName, CContainerUI::st_str_childpadding ) == 0 ) SetChildPadding(_ttoi(pstrValue));
...
//
//
const CStdString CVerticalLayoutUI::st_str_sepheight		=_T("sepheight");
const CStdString CVerticalLayoutUI::st_str_sepimm			=_T("sepimm");
...
{
    if( _tcscmp(pstrName, CVerticalLayoutUI::st_str_sepheight ) == 0 ) SetSepHeight(_ttoi(pstrValue));
    else if( _tcscmp(pstrName, CVerticalLayoutUI::st_str_sepimm ) == 0 ) SetSepImmMode(_tcscmp(pstrValue, _T("true")) == 0);
    ...
    if( m_iSepHeight != 0 ) {
        ...
        if( event.Type == UIEVENT_MOUSEMOVE )
        {
            if( (m_uButtonState & UISTATE_CAPTURED) != 0 ) {
                ...
                CDirectRect rcInvalidate = GetThumbRect(true);
                m_rcNewPos = rc;
...
RECT CVerticalLayoutUI::GetThumbRect(bool bUseNew) const
{
    if( (m_uButtonState & UISTATE_CAPTURED) != 0 && bUseNew) {
        if( m_iSepHeight >= 0 ) 
            return CDirectRect(m_rcNewPos.left, MAX(m_rcNewPos.bottom - m_iSepHeight, m_rcNewPos.top), 
            m_rcNewPos.right, m_rcNewPos.bottom);
        else 
            return CDirectRect(m_rcNewPos.left, m_rcNewPos.top, m_rcNewPos.right, 
            MIN(m_rcNewPos.top - m_iSepHeight, m_rcNewPos.bottom));
    }
    else {
        if( m_iSepHeight >= 0 ) 
            return CDirectRect(m_rcItem.left, MAX(m_rcItem.bottom - m_iSepHeight, m_rcItem.top), m_rcItem.right, 
            m_rcItem.bottom);
        else 
            return CDirectRect(m_rcItem.left, m_rcItem.top, m_rcItem.right, 
...
//
//
const CStdString	CHorizontalLayoutUI::st_str_sepwidth		=_T("sepwidth");
const CStdString	CHorizontalLayoutUI::st_str_sepimm			=_T("sepimm");
...
{
    if( _tcscmp(pstrName, CHorizontalLayoutUI::st_str_sepwidth ) == 0 ) SetSepWidth(_ttoi(pstrValue));
    else if( _tcscmp(pstrName, CHorizontalLayoutUI::st_str_sepimm ) == 0 ) SetSepImmMode(_tcscmp(pstrValue, _T("true")) == 0);
    else CContainerUI::SetAttribute(pstrName, pstrValue);
}
...
{
    if( (m_uButtonState & UISTATE_CAPTURED) != 0 && bUseNew) {
        if( m_iSepWidth >= 0 ) return CDirectRect(m_rcNewPos.right - m_iSepWidth, m_rcNewPos.top, m_rcNewPos.right, m_rcNewPos.bottom);
        else return CDirectRect(m_rcNewPos.left, m_rcNewPos.top, m_rcNewPos.left - m_iSepWidth, m_rcNewPos.bottom);
    }
    else {
        if( m_iSepWidth >= 0 ) return CDirectRect(m_rcItem.right - m_iSepWidth, m_rcItem.top, m_rcItem.right, m_rcItem.bottom);
        else return CDirectRect(m_rcItem.left, m_rcItem.top, m_rcItem.left - m_iSepWidth, m_rcItem.bottom);
    }
}
...
//
//
const CStdString CTileLayoutUI::st_str_columns 			=_T("columns");
...
{
    if( _tcscmp(pstrName, CTileLayoutUI::st_str_columns ) == 0 ) SetColumns(_ttoi(pstrValue));
    else CContainerUI::SetAttribute(pstrName, pstrValue);
}
...
RECT CDialogLayoutUI::RecalcArea()
{
    ...
    CDirectRect rcDialog(9999, 9999, 0,0);
    for( int i = 0; i < m_aModes.GetSize(); i++ ) {
        STRETCHMODE* pItem = static_cast<STRETCHMODE*>(m_aModes[i]);
        if( !pItem->pControl->IsVisible() ) continue;
...
//
//
const CStdString CTabLayoutUI::st_str_selectedid		=_T("selectedid");
...
bool CTabLayoutUI::SelectItem(int iIndex)
{
    ...
    if( m_pManager != NULL ) {
        ...
        m_pManager->SendNotify(this, NOTIFY_TABLESELECT, m_iCurSel, iOldSel);
    }
    return true;
}
...
{
    if( _tcscmp(pstrName, CTabLayoutUI::st_str_selectedid ) == 0 ) SelectItem(_ttoi(pstrValue));
