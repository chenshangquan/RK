...
STDMETHODIMP CActiveXCtrl::GetDC(LPCRECT pRect, DWORD grfFlags, HDC* phDC)
{
    ...
    *phDC = ::GetDC(m_pOwner->m_hwndHost);
    if( (grfFlags & OLEDC_PAINTBKGND) != 0 ) {
        CDirectRect rcItem = m_pOwner->GetPos();
...
STDMETHODIMP CActiveXCtrl::OnInPlaceActivateEx(BOOL* pfNoRedraw, DWORD dwFlags)        
{
    ...
    if( m_pInPlaceObject != NULL ) {
        CDirectRect rcItem = m_pOwner->m_rcItem;
        if( !m_bWindowless ) rcItem.ResetOffset();
...
//
//
const CStdString CActiveXUI::st_str_clsid					=_T("clsid");
const CStdString CActiveXUI::st_str_modulename				=_T("modulename");
const CStdString CActiveXUI::st_str_delaycreate				=_T("delaycreate");
...
{
    if( _tcscmp(pstrName, CActiveXUI::st_str_clsid) == 0 ) CreateControl(pstrValue);
    else if( _tcscmp(pstrName, CActiveXUI::st_str_modulename ) == 0 ) SetModuleName(pstrValue);
    else if( _tcscmp(pstrName, CActiveXUI::st_str_delaycreate ) == 0 ) SetDelayCreate(_tcscmp(pstrValue, _T("true")) == 0);
...
bool CActiveXUI::DoCreateControl()
{
    ...
    if( m_pManager != NULL ) m_pManager->SendNotify((CControlUI*)this, NOTIFY_SHOWACTIVEX, 0, 0, false);
