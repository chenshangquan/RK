...
void __Trace(LPCTSTR pstrFormat, ...)
{
	...
	_snprintf( szBuffer, sizeof(szBuffer) - 2, pstrFormat, args );
    _tcscat(szBuffer, _T("\n"));
    va_end(args);
...
CDirectPoint::CDirectPoint()
{
    x = y = 0;
}

CDirectPoint::CDirectPoint(const POINT& src)
{
    x = src.x;
    y = src.y;
}

CDirectPoint::CDirectPoint(int _x, int _y)
{
    x = _x;
    y = _y;
}

CDirectPoint::CDirectPoint(LPARAM lParam)
{
...
CDirectSize::CDirectSize()
{
    cx = cy = 0;
}

CDirectSize::CDirectSize(const SIZE& src)
{
    cx = src.cx;
    cy = src.cy;
}

CDirectSize::CDirectSize(const RECT rc)
{
    cx = rc.right - rc.left;
    cy = rc.bottom - rc.top;
}

CDirectSize::CDirectSize(int _cx, int _cy)
{
    cx = _cx;
    cy = _cy;
}
...
CDirectRect::CDirectRect(const RECT& src)
{
    left = src.left;
    top = src.top;
    right = src.right;
...
CDirectRect::CDirectRect(int iLeft, int iTop, int iRight, int iBottom)
{
    left = iLeft;
    top = iTop;
    right = iRight;
...
int CDirectRect::GetWidth() const
{
    return right - left;
}

int CDirectRect::GetHeight() const
{
    return bottom - top;
}

void CDirectRect::Empty()
{
    left = top = right = bottom = 0;
}

bool CDirectRect::IsNull() const
{
    return (left == 0 && right == 0 && top == 0 && bottom == 0); 
}

void CDirectRect::Join(const RECT& rc)
{
...
void CDirectRect::ResetOffset()
{
    ::OffsetRect(this, -left, -top);
}

void CDirectRect::Normalize()
{
    if( left > right ) { int iTemp = left; left = right; right = iTemp; }
    if( top > bottom ) { int iTemp = top; top = bottom; bottom = iTemp; }
}

void CDirectRect::Offset(int cx, int cy)
{
    ::OffsetRect(this, cx, cy);
}

void CDirectRect::Inflate(int cx, int cy)
{
    ::InflateRect(this, cx, cy);
}

void CDirectRect::Deflate(int cx, int cy)
{
    ::InflateRect(this, -cx, -cy);
}

void CDirectRect::Union(CDirectRect& rc)
{
