...
//
//
const CStdString CLabelUI::st_str_align				=_T("align");
const CStdString CLabelUI::st_str_endellipsis		=_T("endellipsis");
const CStdString CLabelUI::st_str_font				=_T("font");
const CStdString CLabelUI::st_str_textcolor			=_T("textcolor");
const CStdString CLabelUI::st_str_disabledtextcolor	=_T("disabledtextcolor");
const CStdString CLabelUI::st_str_textpadding		=_T("textpadding");
const CStdString CLabelUI::st_str_showhtml			=_T("showhtml");
...
{
    if( m_cxyFixed.cy == 0 ) return CDirectSize(m_cxyFixed.cx, m_pManager->GetDefaultFontInfo()->tm.tmHeight + 4);
...
{
    if( _tcscmp(pstrName, CLabelUI::st_str_align ) == 0 ) {
    ...
    else if( _tcscmp(pstrName, CLabelUI::st_str_endellipsis ) == 0 ) {
        if( _tcscmp(pstrValue, _T("true")) == 0 ) m_uTextStyle |= DT_END_ELLIPSIS;
        else m_uTextStyle &= ~DT_END_ELLIPSIS;
    }    
    else if( _tcscmp(pstrName, CLabelUI::st_str_font ) == 0 ) SetFont(_ttoi(pstrValue));
    else if( _tcscmp(pstrName, CLabelUI::st_str_textcolor ) == 0 ) {
        if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
        LPTSTR pstr = NULL;
        DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
        SetTextColor(clrColor);
    }
    else if( _tcscmp(pstrName, CLabelUI::st_str_disabledtextcolor ) == 0 ) {
        if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
        LPTSTR pstr = NULL;
        DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
        SetDisabledTextColor(clrColor);
    }
    else if( _tcscmp(pstrName, CLabelUI::st_str_textpadding ) == 0 ) {
    ...
    else if( _tcscmp(pstrName, CLabelUI::st_str_showhtml ) == 0 ) SetShowHtml(_tcscmp(pstrValue, _T("true")) == 0);
...
void CButtonUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_CONTEXTMENU )
    {
        if( IsContextMenuUsed() ) {
            m_pManager->SendNotify(this, NOTIFY_MENU, event.wParam, event.lParam);
        }
        return;
    }
    if( event.Type == UIEVENT_MOUSEENTER )
...
bool CButtonUI::Activate()
{
    ...
    if( m_pManager != NULL ) m_pManager->SendNotify(this, NOTIFY_CLICK);
    return true;
}
...
{
    if( m_cxyFixed.cy == 0 ) return CDirectSize(m_cxyFixed.cx, m_pManager->GetDefaultFontInfo()->tm.tmHeight + 8);
    return CControlUI::EstimateSize(szAvailable);
}
    ...
    if( m_pManager != NULL ) {
        if( !m_sGroupName.IsEmpty() ) {
            if( m_bSelected ) {
                ...
                m_pManager->SendNotify(this, NOTIFY_SELECTCHANGED );
            }
        }
        else {
            m_pManager->SendNotify(this, NOTIFY_SELECTCHANGED);
...
void CTextUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_BUTTONUP && IsEnabled() ) {
        for( int i = 0; i < m_nLinks; i++ ) {
            if( ::PtInRect(&m_rcLinks[i], event.ptMouse) ) {
                m_pManager->SendNotify(this, NOTIFY_LINK , i);
                return;
            }
        }
    }
...
RECT CSliderUI::GetThumbRect() const
{
    if( m_bHorizontal ) {
        ...
        return CDirectRect(left, top, left + m_szThumb.cx, top + m_szThumb.cy); 
    }
    else {
...
void CSliderUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_BUTTONUP )
    {
        ...
        m_pManager->SendNotify(this, NOTIFY_VALUECHANGED );
        Invalidate();
        return;
    }
    if( event.Type == UIEVENT_CONTEXTMENU )
...
void CEditWnd::Init(CEditUI* pOwner)
{
	...
	if( m_pOwner->IsNumberOnly() ) uStyle |= ES_NUMBER;
...
{
    CDirectRect rcPos = m_pOwner->GetPos();
...
LRESULT CEditWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if( uMsg == WM_KILLFOCUS ) lRes = OnKillFocus(uMsg, wParam, lParam, bHandled);
        ...
        m_pOwner->GetManager()->SendNotify(m_pOwner, NOTIFY_RETURN );
    }
...
LRESULT CEditWnd::OnEditChanged(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
    ...
    m_pOwner->GetManager()->SendNotify(m_pOwner, NOTIFY_TEXTCHANGED );
    return 0;
}
...
m_bPasswordMode(false), m_cPasswordChar(_T('*')), m_uButtonState(0), m_dwEditbkColor(0xFFFFFFFF), m_bNumberOnly(false)
{
    SetTextPadding(CDirectRect(4, 3, 4, 3));
...
{
	if ( m_bNumberOnly == bNumberOnly ) return;
...
{
    if( m_cxyFixed.cy == 0 ) return CDirectSize(m_cxyFixed.cx, m_pManager->GetDefaultFontInfo()->tm.tmHeight + 6);
    return CControlUI::EstimateSize(szAvailable);
}
...
//
//
const CStdString CScrollBarUI::st_str_button1normalimage		=_T("button1normalimage");
const CStdString CScrollBarUI::st_str_button1hotimage			=_T("button1hotimage");
const CStdString CScrollBarUI::st_str_button1pushedimage		=_T("button1pushedimage");
const CStdString CScrollBarUI::st_str_button1disabledimage		=_T("button1disabledimage");
const CStdString CScrollBarUI::st_str_button2normalimage		=_T("button2normalimage");
const CStdString CScrollBarUI::st_str_button2hotimage			=_T("button2hotimage");
const CStdString CScrollBarUI::st_str_button2pushedimage		=_T("button2pushedimage");
const CStdString CScrollBarUI::st_str_button2disabledimage		=_T("button2disabledimage");
const CStdString CScrollBarUI::st_str_thumbnormalimage			=_T("thumbnormalimage");
const CStdString CScrollBarUI::st_str_thumbhotimage				=_T("thumbhotimage");
const CStdString CScrollBarUI::st_str_thumbpushedimage			=_T("thumbpushedimage");
const CStdString CScrollBarUI::st_str_thumbdisabledimage		=_T("thumbdisabledimage");
const CStdString CScrollBarUI::st_str_railnormalimage			=_T("railnormalimage");
const CStdString CScrollBarUI::st_str_railhotimage				=_T("railhotimage");
const CStdString CScrollBarUI::st_str_railpushedimage			=_T("railpushedimage");
const CStdString CScrollBarUI::st_str_raildisabledimage			=_T("raildisabledimage");
const CStdString CScrollBarUI::st_str_bknormalimage				=_T("bknormalimage");
const CStdString CScrollBarUI::st_str_bkhotimage				=_T("bkhotimage");
const CStdString CScrollBarUI::st_str_bkpushedimage				=_T("bkpushedimage");
const CStdString CScrollBarUI::st_str_bkdisabledimage			=_T("bkdisabledimage");
const CStdString CScrollBarUI::st_str_hor						=_T("hor");
const CStdString CScrollBarUI::st_str_linesize					=_T("linesize");
const CStdString CScrollBarUI::st_str_range						=_T("range");
const CStdString CScrollBarUI::st_str_value						=_T("value");
const CStdString CScrollBarUI::st_str_showbutton1				=_T("showbutton1");
const CStdString CScrollBarUI::st_str_showbutton2				=_T("showbutton2");
...
void CScrollBarUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK )
    {
        ...
        if( m_pManager != NULL && m_pOwner == NULL ) m_pManager->SendNotify(this, NOTIFY_SCROLL);
        return;
    }
    if( event.Type == UIEVENT_BUTTONUP )
    {
    ...
    if( event.Type == UIEVENT_TIMER && event.wParam == DEFAULT_TIMERID )
    {
        ...
        if( (m_uThumbState & UISTATE_CAPTURED) != 0 ) {
            if( !m_bHorizontal ) {
                if( m_pOwner != NULL ) m_pOwner->SetScrollPos(CDirectSize(m_pOwner->GetScrollPos().cx, \
                    m_nLastScrollPos + m_nLastScrollOffset)); 
                else SetScrollPos(m_nLastScrollPos + m_nLastScrollOffset);
            }
            else {
                if( m_pOwner != NULL ) m_pOwner->SetScrollPos(CDirectSize(m_nLastScrollPos + m_nLastScrollOffset, \
...
{
    if( _tcscmp(pstrName, CScrollBarUI::st_str_button1normalimage ) == 0 ) SetButton1NormalImage(pstrValue);
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_button1hotimage ) == 0 ) SetButton1HotImage(pstrValue);
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_button1pushedimage ) == 0 ) SetButton1PushedImage(pstrValue);
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_button1disabledimage ) == 0 ) SetButton1DisabledImage(pstrValue);
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_button2normalimage ) == 0 ) SetButton2NormalImage(pstrValue);
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_button2hotimage ) == 0 ) SetButton2HotImage(pstrValue);
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_button2pushedimage ) == 0 ) SetButton2PushedImage(pstrValue);
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_button2disabledimage ) == 0 ) SetButton2DisabledImage(pstrValue);
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_thumbnormalimage ) == 0 ) SetThumbNormalImage(pstrValue);
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_thumbhotimage ) == 0 ) SetThumbHotImage(pstrValue);
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_thumbpushedimage ) == 0 ) SetThumbPushedImage(pstrValue);
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_thumbdisabledimage ) == 0 ) SetThumbDisabledImage(pstrValue);
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_railnormalimage ) == 0 ) SetRailNormalImage(pstrValue);
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_railhotimage ) == 0 ) SetRailHotImage(pstrValue);
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_railpushedimage ) == 0 ) SetRailPushedImage(pstrValue);
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_raildisabledimage ) == 0 ) SetRailDisabledImage(pstrValue);
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_bknormalimage ) == 0 ) SetBkNormalImage(pstrValue);
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_bkhotimage ) == 0 ) SetBkHotImage(pstrValue);
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_bkpushedimage ) == 0 ) SetBkPushedImage(pstrValue);
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_bkdisabledimage ) == 0 ) SetBkDisabledImage(pstrValue);
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_hor ) == 0 ) SetHorizontal(_tcscmp(pstrValue, _T("true")) == 0);
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_linesize ) == 0 ) SetLineSize(_ttoi(pstrValue));
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_range ) == 0 ) SetScrollRange(_ttoi(pstrValue));
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_value ) == 0 ) SetScrollPos(_ttoi(pstrValue));
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_showbutton1 ) == 0 ) SetShowButton1(_tcscmp(pstrValue, _T("true")) == 0);
    else if( _tcscmp(pstrName, CScrollBarUI::st_str_showbutton2 ) == 0 ) SetShowButton2(_tcscmp(pstrValue, _T("true")) == 0);
    else CControlUI::SetAttribute(pstrName, pstrValue);
}
