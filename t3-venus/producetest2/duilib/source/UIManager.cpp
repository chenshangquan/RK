...
CPaintManagerUI::~CPaintManagerUI()
{
    ...
    for( i = 0; i < m_aAsyncNotify.GetSize(); i++ ) delete static_cast<TNotifyUI*>(m_aAsyncNotify[i]);
...
SIZE CPaintManagerUI::GetClientSize() const
{
    ...
    return CDirectSize(rcClient.right - rcClient.left, rcClient.bottom - rcClient.top);
}
            ...
            if( m_bUpdateNeeded ) {
                ...
                if( !::IsRectEmpty(&rcClient) ) {
                    ...
                    if( m_bFirstLayout ) {
                        ...
                        SendNotify(m_pRoot, NOTIFY_INIWINDOW,  0, 0, false);
                    }
                }
            }
...
void CPaintManagerUI::SetFocus(CControlUI* pControl)
{
    ...
    if( m_pFocus != NULL ) 
    {
        ...
        event.dwTimestamp = ::GetTickCount();
        m_pFocus->Event(event);
        SendNotify(m_pFocus, NOTIFY_KILLFOCUS );
        m_pFocus = NULL;
    }
        ...
        && pControl->IsEnabled() ) 
    {
        ...
        event.dwTimestamp = ::GetTickCount();
        m_pFocus->Event(event);
        SendNotify(m_pFocus, NOTIFY_SETFOCUS );
    }
}
